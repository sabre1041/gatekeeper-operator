...
@test "required labels dryrun test" {
  ...
  num_whpods=$(kubectl -n gatekeeper-system get pod -l gatekeeper.sh/operation=webhook -o json | jq '.items | length')
  run wait_for_process $WAIT_TIME $SLEEP_TIME "compare_count k8srequiredlabels ns-must-have-gk ${num_whpods}"
  assert_success
...
@test "container limits test" {
  ...
  num_whpods=$(kubectl -n gatekeeper-system get pod -l gatekeeper.sh/operation=webhook -o json | jq '.items | length')
  run wait_for_process $WAIT_TIME $SLEEP_TIME "compare_count k8scontainerlimits container-must-have-limits ${num_whpods}"
  assert_success
  ...
  assert_success
  CLEAN_CMD+="; kubectl delete -f ${BATS_TESTS_DIR}/templates/k8scontainterlimits_template.yaml"
  CLEAN_CMD+="; kubectl delete -f ${BATS_TESTS_DIR}/good/good_ns.yaml"
}
...
@test "unique labels test" {
  ...
  assert_success
  CLEAN_CMD+="; kubectl delete -f ${BATS_TESTS_DIR}/templates/k8suniquelabel_template.yaml"
  ...
  num_whpods=$(kubectl -n gatekeeper-system get pod -l gatekeeper.sh/operation=webhook -o json | jq '.items | length')
  run wait_for_process $WAIT_TIME $SLEEP_TIME "compare_count k8suniquelabel ns-gk-label-unique ${num_whpods}"
  assert_success

  run kubectl apply -f ${BATS_TESTS_DIR}/bad/no_dupe_ns_2.yaml
  assert_match 'denied the request' "$output"
  assert_failure
  CLEAN_CMD+="; kubectl delete -f ${BATS_TESTS_DIR}/good/no_dupe_ns.yaml"
}
...
@test "required labels audit test" {
  ...
  CLEAN_CMD+="; kubectl delete -f ${BATS_TESTS_DIR}/templates/k8srequiredlabels_template.yaml"
}
...
@test "config namespace exclusion test" {
  ...
  assert_success
  CLEAN_CMD+="; kubectl delete ns excluded-namespace"
  CLEAN_CMD+="; kubectl delete -f ${BATS_TESTS_DIR}/sync.yaml"
